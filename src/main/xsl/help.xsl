...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml" version="1.0">
  ...
  <xsl:template match="page" mode="body">
    ...
    <p>
      <code>
        <xsl:text>domain add www.thindeck.com</xsl:text>
      </code>
    ...
    <p>
      <code>
        <xsl:text>domain remove www.thindeck.com</xsl:text>
      </code>
      <xsl:text> removes a domain from our load balancer.</xsl:text>
    </p>
    <p>
      <code>
        <xsl:text>repo put git@github.com:jeff/foo</xsl:text>
      </code>
      ...
      <code>
        <xsl:text>git@github.com:jeff/foo#master:src/docker</xsl:text>
      </code>
    ...
    <p>
      <code>
        <xsl:text>container waste a1b2c3d4</xsl:text>
      </code>
      <xsl:text> marks container </xsl:text>
      <code>
        <xsl:text>a1b2c3d4</xsl:text>
      </code>
    ...
    <p>
      <code>
        <xsl:text>image waste foo/foo-a1b2c3d4</xsl:text>
      </code>
      <xsl:text> marks image </xsl:text>
      <code>
        <xsl:text>foo/foo-a1b2c3d4</xsl:text>
      </code>
      <xsl:text> as waste. It will be automatically terminated and</xsl:text>
      <xsl:text> destroyed soon.</xsl:text>
    </p>
