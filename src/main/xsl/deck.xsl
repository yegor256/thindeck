...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml" version="1.0">
  ...
  <xsl:template match="deck">
    ...
    <form action="{links/link[@rel='command']/@href}" method="get">
      <fieldset>
        <input type="text" name="command" size="50" maxlength="1000" placeholder="tell me what to do..."/>
    ...
    <table>
      ...
      <xsl:for-each select="image">
        <tr>
          ...
          <td>
            <a href="{/page/deck/links/link[@rel='command']/@href}?command=repo+put+{uri}" title="re-deploy it">
              <xsl:text>⟳</xsl:text>
            </a>
            <xsl:text> </xsl:text>
            <a href="{/page/deck/links/link[@rel='command']/@href}?command=image+waste+{name}" title="waste it">
              <xsl:text>✘</xsl:text>
            </a>
          </td>
        </tr>
